...
double figure[1000][7];
char shape[1000];

double direction (double a, double b, double c, double d) {
    return a*d - b*c;
}
...
bool inTriangle (double x1, double y1, double x2, double y2, double x3, double y3, double x, double y) {
    double d1 = direction(x-x1, y-y1, x3-x1, y3-y1);
    double d2 = direction(x-x1, y-y1, x2-x1, y2-y1);
    double d3 = direction(x-x2, y-y2, x1-x2, y1-y2);
    double d4 = direction(x-x2, y-y2, x3-x2, y3-y2);
    ...
    while (ch != '*') {
        ...
        while (ch != '*') {
            if (ch == 'r'){
                ...
                scanf("%lf %lf %lf %lf %lf %lf", &figure[figures][0], &figure[figures][1], &figure[figures][2], &figure[figures][3], &figure[figures][4], &figure[figures][5]);
            }
            shape[shapes++] = ch;
            figures++;
            gets(dump);
        ...
        while (flag) {
            ...
            for (int i=0; i<figures && flag; i++){
                if (shape[i] == 'r') {
                    ...
                    if (inTriangle(figure[i][0], figure[i][1], figure[i][2], figure[i][3], figure[i][4], figure[i][5], x, y)) {
                        printf("Point %d is contained in figure %d\n", points, i+1);
                        contained = true;
                    }
                }
